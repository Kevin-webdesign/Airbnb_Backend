...
async function canManageListingPhotos(req, listingId) {
...
export async function uploadListingPhotos(req, res) {
    ...
    const uploaded = await Promise.all(files.map((file) => uploadToCloudinary(file.buffer, "airbnb/listings")));
    const photos = await prisma.$transaction(uploaded.map(({ url, publicId }) => prisma.listingPhoto.create({
        data: {
            url,
            publicId,
            listingId,
