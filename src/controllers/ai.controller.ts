...
interface AuthRequest<P = Record<string, unknown>> extends Request<P> {
  userId?: string;
  role?: string;
}
...
`);
  ...
  const validation = await validationChain.invoke({ query }) as {
...
export async function explainListing(req: Request<{ listingId: string }>, res: Response) {
